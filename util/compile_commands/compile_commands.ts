...
const p = child_process.spawn("make", {
	env: {
		...process.env,
		VERBOSE: "ON",
	},
...
const replace_current_file = () => {
	...
	console.log("\x1b[36m%s\x1b[0m", "Successfully generated compile_commands.json")
}
...
p.on("close", (code) => {
	if (code !== 0) {
		console.error(err)
		console.error("\x1b[31m%s\x1b[0m. Error code: %d", "Make did not exit successfully!", code)
	} else {
		finalize()
	}
})
